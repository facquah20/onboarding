...
public class MyOnboardingView extends StandardView {
    ...
    @ViewComponent
    private CollectionContainer<UserStep> userStepsDc1;

    @ViewComponent
    private DataContext dataContext;
    @ViewComponent("userStepsDataGrid.dueDate")
    private Grid.Column<UserStep> userStepsDataGridDueDate;
    ...
    private void updateLabels(){
        totalStepsLabel.setText("Total steps: "+userStepsDc1.getItems().size());
        long completedCount = userStepsDc1.getItems().stream()
                .filter(us->us.getCompletedDate() != null)
                .count();
        completedStepsLabel.setText("Completed steps: "+completedCount);

        long overdueCount = userStepsDc1.getItems().stream()
